...
namespace Geco.Common.MetadataProviders
{
    ...
    {
        private static readonly ConcurrentDictionary<Type, Delegate> MaterializeFunctions =
            new ConcurrentDictionary<Type, Delegate>();


        private static readonly MethodInfo ReadValueOrDefaultMethod = typeof(QueryUtil).GetTypeInfo()
            .GetMethods(BindingFlags.NonPublic | BindingFlags.Static).First(m => m.Name == nameof(ReadValueOrDefault));

        public static IEnumerable<T> MaterializeReader<T>(DbDataReader reader)
            where T : IMetadataItem, new()
        {
            var materialize = GetCachedMaterializeFunc<T>();
            while (reader.Read()) yield return materialize(reader);
        }
            ...
            where T : IMetadataItem
        {
            return (Func<DbDataReader, T>) MaterializeFunctions.GetOrAdd(typeof(T), t =>
            {
                ...
                var exceptProperties = new HashSet<string>(t.GetProperties().Select(p => p.Name)
                    .Where(p => nameof(IMetadataItem.Metadata) != p && nameof(IMetadataItem.FullyQualifiedName) != p));
                var lambda = Expression.Lambda(
                    Expression.Call(
                        typeof(QueryUtil).GetMethod(nameof(AddMetadata), BindingFlags.Static | BindingFlags.NonPublic)
                            .MakeGenericMethod(t),
                        Expression.MemberInit
                        (
                            Expression.New(t),
                            t.GetTypeInfo().GetProperties()
                                .Where(p => nameof(IMetadataItem.Metadata) != p.Name && nameof(IMetadataItem.FullyQualifiedName) != p.Name)
                                .Where(p => p.CanWrite)
        ...
        private static Expression GetAssignmentExpression(ParameterExpression r, PropertyInfo p)
        {
            ...
            return Expression.Call(ReadValueOrDefaultMethod.MakeGenericMethod(p.PropertyType),
                EnumerableExtensions.Yield<Expression>(r, Expression.Constant(p.Name)));
        }
        ...
        private static T ReadValueOrDefault<T>(DbDataReader r, string name)
        {
            ...
            return r.IsDBNull(index) ? default : r.GetFieldValue<T>(index);
        }
    }
}